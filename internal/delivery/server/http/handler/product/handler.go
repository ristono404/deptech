...
func New(service productUsecase.Service) *Handler {
	return &Handler{
		Service: service,
	}
}
...
func (h *Handler) Read(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		response.Response(w, response.View{
			Message: "failed to read the product",
		}, http.StatusInternalServerError)
...
func (h *Handler) In(w http.ResponseWriter, r *http.Request) {
	...
	var body []event.TransactionCreated
	err := json.NewDecoder(r.Body).Decode(&body)
	if err != nil {
		log.Println(err)
		response.Response(w, response.View{
...
func (h *Handler) TransactionHistory(w http.ResponseWriter, r *http.Request) {
	...
	data, err := h.Service.TransactionHistory()
